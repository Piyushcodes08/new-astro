...
const ArticlePage = () => {
  const { slug } = useParams(); // ✅ Matches your route param name
    ...
    const fetchArticle = async () => {
      try {
        ...
        const matched = snapshot.docs
          ...
          .find((item) => item.slug === slug); // 'id' is actually the slug
  ...
  const getMetaTags = () => {
    ...
    return (
      <Helmet>
        <title>{article?.sTitle || 'Default Title'} - Astrology Articles by Valay Patel</title>
        <meta name="description" content={article?.sDesc || 'Default description'} />
        <meta
          name="keywords"
          content={
            Array.isArray(article?.sKeywords)
              ? article.sKeywords.join(', ')
              : article?.sKeywords || 'default, keywords'
          }
        />
      </Helmet>
